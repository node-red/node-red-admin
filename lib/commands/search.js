...
function command(argv,result) {
    ...
    httpRequest(options, function(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            result.log(error.toString().red);
        } else {
            result.log((response.statusCode + ": " + body).red);
        }
    });

}
command.name = "search";
command.usage = command.name+" <search-term> [options]";
command.description = "Searches NPM for Node-RED modules relating to the search-term given";
