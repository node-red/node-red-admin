    ...
    return when.promise(function(resolve) {
        ...
        httpRequest.get(options, function(error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                var found = false;
                result.log(objectsInfo.total + ' objects found')

                for (var i = 0; i < objectsInfo.total; i++) {
                    var objectsEntry = objectsInfo.objects[i];
                    result.log(objectsEntry.package.name)

                    if (!filter || filter.test(objectsEntry.package.name) || filter.test(objectsEntry.package.keywords)) {
